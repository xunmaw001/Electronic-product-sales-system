<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="产品类别" prop="chanpinleibie">
					<el-input v-model="ruleForm.chanpinleibie" placeholder="产品类别" clearable  :readonly="ro.chanpinleibie"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="产品类别" prop="chanpinleibie">
					<el-input v-model="ruleForm.chanpinleibie" placeholder="产品类别" readonly></el-input>
				</el-form-item>
			</template>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chanpinleibie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chanpinleibieCrossAddOrUpdateFlag = false;
						 this.parent.search();
